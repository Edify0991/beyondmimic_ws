...
def setup_controllers(context):
    ...
    reference_path_value = LaunchConfiguration('reference_path').perform(context)
    reference_loop_value = LaunchConfiguration('reference_loop').perform(context)
    ...
    if policy_path_value:
        ...
        raise RuntimeError(
            "No policy_path resolved. Provide policy_path:=<local_onnx_path> "
            "or enable use_wandb:=true with wandb_path:=<entity/project/run_id>."
        )
    if start_step_value:
        kv_pairs.append(('walking_controller.motion.start_step', start_step_value))
    if reference_path_value:
        abs_reference_path = os.path.abspath(os.path.expanduser(os.path.expandvars(reference_path_value)))
        kv_pairs.append(('walking_controller.motion.reference_path', abs_reference_path))
    if reference_loop_value:
        kv_pairs.append(('walking_controller.motion.reference_loop', reference_loop_value))
...
def generate_launch_description():
    ...
    wandb = IncludeLaunchDescription(
        ...
        condition=IfCondition(LaunchConfiguration('use_wandb'))
    )
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            'use_wandb',
            default_value='false',
            description='Set true to download ONNX from WandB before launching.'
        ),
        DeclareLaunchArgument(
            'wandb_path',
            default_value='',
            description='WandB run path: <entity>/<project>/<run_id>. Used only when use_wandb:=true.'
        ),
        DeclareLaunchArgument(
            'policy_path',
            default_value='',
        ...
        DeclareLaunchArgument(
            'reference_path',
            default_value='',
            description='Absolute or ~-expanded local reference motion file path.'
        ),
        DeclareLaunchArgument(
            'reference_loop',
            default_value='true',
            description='Loop local reference frames when end-of-file is reached.'
        ),
        DeclareLaunchArgument(
            'ext_pos_corr',
            default_value='false',
