...
controller_interface::CallbackReturn MotionTrackingController::on_init() {
  ...
  try {
    ...
    auto_declare("motion.reference_path", "");
    auto_declare("motion.reference_loop", true);
...
controller_interface::CallbackReturn MotionTrackingController::on_configure(const rclcpp_lifecycle::State& previous_state) {
  ...
  const auto referencePath = get_node()->get_parameter("motion.reference_path").as_string();
  const auto referenceLoop = get_node()->get_parameter("motion.reference_loop").as_bool();

  policy_ = std::make_shared<MotionOnnxPolicy>(policyPath, startStep, referencePath, referenceLoop);
  policy_->init();
  ...
  if (!referencePath.empty()) {
    RCLCPP_INFO_STREAM(rclcpp::get_logger("MotionTrackingController"),
                       "Use local reference motion from " << referencePath
                                                         << " (loop=" << (referenceLoop ? "true" : "false") << ")");
  }
