...
class MotionOnnxPolicy : public OnnxPolicy {
 public:
  ...
  MotionOnnxPolicy(const std::string& modelPath, size_t startStep, std::string referencePath = "", bool loopLocalReference = true)
      : OnnxPolicy(modelPath),
        startStep_(startStep),
        localReferencePath_(std::move(referencePath)),
        loopLocalReference_(loopLocalReference) {}
 ...
 protected:
  struct LocalReferenceFrame {
    ...
    std::vector<vector3_t> bodyPositions;
    std::vector<quaternion_t> bodyOrientations;
  };

  void loadLocalReferenceIfNeeded();
  ...
  std::string localReferencePath_;
  bool loopLocalReference_ = true;
  bool localReferenceLoaded_ = false;
  std::vector<LocalReferenceFrame> localReferenceFrames_;
};
